...
def ask_question(question, session_id):
    ...
    answer = ""
    chunks = list(get_llm().stream(qa_prompt))  # Lấy toàn bộ chunk trước

    for i, chunk in enumerate(chunks):
        content = chunk.content
        if i < len(chunks) - 2:  # Nếu không phải phần cuối cùng
            content = content.replace("\n", " ")  # Thay thế \n thành dấu cách
        answer += content  # Nối phần nội dung vào answer
